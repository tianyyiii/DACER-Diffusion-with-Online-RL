...
from relax.algorithm.sdac_soft import SDAC_Soft as SDAC_Soft_v1
from relax.algorithm.sdac_soft_v2 import SDAC_Soft as SDAC_Soft_v2
...
from relax.network.sdac_soft import create_sdac_net_soft as create_sdac_net_soft_v1
from relax.network.sdac_soft_v2 import create_sdac_net_soft as create_sdac_net_soft_v2
...
if __name__ == "__main__":
    ...
    elif args.alg == 'sdac_soft_v1':
        action_low = env.action_space.low  
        action_high = env.action_space.high  
        action_range = jnp.array([action_low[0], action_high[0]])
        def mish(x: jax.Array):
            return x * jnp.tanh(jax.nn.softplus(x))
        agent, params = create_sdac_net_soft_v1(init_network_key, obs_dim, act_dim, hidden_sizes, diffusion_hidden_sizes, mish,
                                          num_timesteps=args.diffusion_steps, 
                                          num_particles=args.num_particles, 
                                          noise_scale=args.noise_scale,
                                          target_entropy_scale=args.target_entropy_scale)
        algorithm = SDAC_Soft_v1(agent, params, action_range, lr=args.lr, alpha_lr=args.alpha_lr, 
                           delay_alpha_update=args.delay_alpha_update,
                             lr_schedule_end=args.lr_schedule_end,
                             use_ema=args.use_ema_policy, beta=args.beta, exp_clip=args.exp_clip, value_action_noise=args.value_action_noise)
    elif args.alg == 'sdac_soft_v2':
        def mish(x: jax.Array):
            return x * jnp.tanh(jax.nn.softplus(x))
        agent, params = create_sdac_net_soft_v2(init_network_key, obs_dim, act_dim, hidden_sizes, diffusion_hidden_sizes, mish,
                                          num_timesteps=args.diffusion_steps, 
                                          num_particles=args.num_particles, 
                                          noise_scale=args.noise_scale,
                                          target_entropy_scale=args.target_entropy_scale)
