...
class SDACNet:
    ...
    def get_noisy_action(self, key: jax.Array, policy_params: hk.Params, obs: jax.Array) -> jax.Array:
        policy_params, log_alpha, q1_params, q2_params = policy_params
        ...
        key, noise_key, diffusion_time_key, diffusion_noise_key = jax.random.split(key, 4)
        if self.num_particles == 1:
            act = sample(key)
        else:
            keys = jax.random.split(key, self.num_particles)
            ...
            act = jnp.take_along_axis(acts, q_best_ind[..., None], axis=0).squeeze(axis=0)
        act = act + jax.random.normal(noise_key, act.shape) * jnp.exp(log_alpha) * self.noise_scale
        noise = jax.random.normal(diffusion_noise_key, act.shape)
        t = jax.random.randint(diffusion_time_key, (act.shape[0],), 0, self.num_timesteps)
        act = jax.vmap(self.diffusion.q_sample)(t, act, noise)
        return act
