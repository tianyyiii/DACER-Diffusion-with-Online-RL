...
from relax.network.blocks import Activation, DistributionalQNet2, DACERPolicyNet, QNet, ValueNet
...
def create_sdac_net_soft(
    ...
    value = hk.without_apply_rng(hk.transform(lambda obs: ValueNet(hidden_sizes, activation)(obs)))
    policy = hk.without_apply_rng(hk.transform(lambda obs, act, t: DACERPolicyNet(diffusion_hidden_sizes, activation)(obs, act, t)))

    @jax.jit
    def init(key, obs, act):
        q1_key, q2_key, policy_key, value_key = jax.random.split(key, 4)
        q1_params = q.init(q1_key, obs, act)
        q2_params = q.init(q2_key, obs, act)
        target_q1_params = q1_params
        target_q2_params = q2_params
        value_params = value.init(value_key, obs)
        ...
        return Diffv2Params(q1_params, q2_params, target_q1_params, target_q2_params, value_params, target_value_params, policy_params, target_policy_params, log_alpha)

    sample_obs = jnp.zeros((1, obs_dim))
    sample_act = jnp.zeros((1, act_dim))
    params = init(key, sample_obs, sample_act)

    net = SDACNet(q=q.apply, value=value.apply, policy=policy.apply, num_timesteps=num_timesteps, act_dim=act_dim, 
