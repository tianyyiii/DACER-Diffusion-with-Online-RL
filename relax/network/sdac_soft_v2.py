...
from relax.network.blocks import Activation, DistributionalQNet2, DACERPolicyNet, QNet, PolicyNet
...
class SDACNet:
    ...
    reward_a: Callable[[hk.Params, jax.Array], Tuple[jax.Array, jax.Array]]
    num_timesteps: int
    act_dim: int
    num_particles: int
...
def create_sdac_net_soft(
    ...
    reward_a = hk.without_apply_rng(hk.transform(lambda obs: PolicyNet(1, hidden_sizes, activation)(obs)))

    @jax.jit
    def init(key, obs, act):
        q1_key, q2_key, policy_key, reward_a_key = jax.random.split(key, 4)
    ...
    net = SDACNet(q=q.apply, policy=policy.apply, reward_a=reward_a.apply, num_timesteps=num_timesteps, act_dim=act_dim, 
