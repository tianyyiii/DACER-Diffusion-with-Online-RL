    ...
    def __init__(
        ...
        *,
        ...
        def stateless_update(
            ...
            next_eval_key, action_mask_key, action_uniform_key, diffusion_time_key, diffusion_noise_key, diffusion_time_key1, diffusion_noise_key1 = jax.random.split(
                key, 7)
            ...
            def q_loss_fn(q_params: hk.Params) -> jax.Array:
                # noise_act = jax.random.normal(diffusion_noise_key1, action.shape)
                # t_act = jax.random.randint(diffusion_time_key1, (action.shape[0],), 0, self.agent.num_timesteps)
                # act = jax.vmap(self.agent.diffusion.q_sample)(t_act, action, noise_act)
                # mask = jax.random.bernoulli(action_mask_key, p=0.5, shape=(action.shape[0], 1))
                # uniform_act = jax.random.uniform(
                #     action_uniform_key,
                #     shape=mask.shape,
                #     minval=-1.0,
                #     maxval=1.0
                # )
                # act = jnp.where(mask, action, uniform_act)
            ...
            def policy_loss_fn(policy_params) -> jax.Array:
                next_act = jax.lax.stop_gradient(next_action)

                # mask = jax.random.bernoulli(mask_key, p=0.5, shape=(next_act.shape[0], 1))
                # uniform_act = jax.random.uniform(
                #     uniform_key,
                #     shape=mask.shape,
                #     minval=-1.0,
                #     maxval=1.0
                # )
                # next_act = jnp.where(mask, next_act, uniform_act)

                noise = jax.random.normal(diffusion_noise_key, next_act.shape)
                t = jax.random.randint(diffusion_time_key, (next_obs.shape[0],), 0, self.agent.num_timesteps)
                x_noisy = jax.vmap(self.agent.diffusion.q_sample)(t, next_act, noise)
                q_min = get_min_q(next_obs, x_noisy)
                q_mean, q_std = q_min.mean(), q_min.std()
                norm_q = q_min - running_mean / running_std
                scaled_q = norm_q.clip(-3., 3.) / jnp.exp(log_alpha)
                q_weights = jnp.exp(scaled_q)
                if len(q_weights.shape) == 1:
                    q_weights = q_weights.reshape(-1, 1)
                def denoiser(t, x):
                    return self.agent.policy(policy_params, next_obs, x, t)
                noise_pred = denoiser(t, next_act)
                loss = q_weights * optax.squared_error(noise_pred, noise)
                return loss.mean(), (q_weights, scaled_q, q_mean, q_std)
